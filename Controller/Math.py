...
##Search Yinf
    def searchYinf(self, x, t):
        G = sp.lambdify((sp.Symbol('x'), sp.Symbol('t'), sp.Symbol('z'), sp.Symbol('h')), self.G, modules=['numpy', {'Heaviside': lambda arg: np.heaviside(arg, 1),  'Abs': np.abs}])
        u = sp.lambdify((sp.Symbol('z'), sp.Symbol('h')), self.u, 'numpy')
        Gu = lambda z, h: G(x, t, z, h) * u(z, h)

        return scipy.integrate.dblquad(Gu, 0, self.T, lambda h: self.a, lambda h:  self.b, epsabs=10 ** (-3), epsrel=10 ** (-3))[0]
...
##MAKE A
    ...
    def makeA(self):
        return np.vstack([np.hstack([self.makeAModulMatrix1(), self.makeAModulMatrix1()]), np.hstack([self.makeAModulMatrix2(), self.makeAModulMatrix2()])])
    ...
    def searchIntegralMatrix(self, mat, a, b, t0, T):
        ...
        for i in range(matShape[0]):
            for j in range(matShape[1]):
                ...
                '''
                elemMat=sp.lambdify((sp.Symbol('z'),sp.Symbol('h')), elemMat_expr, modules=['numpy', {'Heaviside': lambda arg: np.heaviside(arg, 1),  'Abs': np.abs}])
                interElemMat = scipy.integrate.dblquad(elemMat, t0, T, lambda h: a, lambda h: b, epsabs=10 ** (-3), epsrel=10 ** (-3))[0]
...
## search Y0 YG and Y
    def searchY0(self,x, t):
        ...
        return scipy.integrate.dblquad(Gu, self.Tm, 0, lambda h: self.a, lambda h: self.b, epsabs=10 ** (-3), epsrel=10 ** (-3))[0]
    ...
    def searchYG(self,x, t):
        ...
        AC = self.a - self.C
        BC = self.b + self.C
        YG2 =scipy.integrate.dblquad(Gu, 0, self.T, lambda h: AC, lambda h: self.a, epsabs=10 ** (-4), epsrel=10 ** (-4))[0]
        YG1 =scipy.integrate.dblquad(Gu, 0, self.T, lambda h: self.b, lambda h: BC, epsabs=10 ** (-3), epsrel=10 ** (-3))[0]
